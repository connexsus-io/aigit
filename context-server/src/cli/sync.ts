...
async function attachVectorEmbeddings(
    ...
    if (tableName !== 'Memory' && tableName !== 'Decision') {
        throw new Error(`Invalid table name for attachVectorEmbeddings: ${tableName}`);
    }
...
export async function dumpContextLedger(workspacePath: string, options: DumpContextLedgerOptions = {}) {
    ...
    try {
        ...
        const [projects, agents, sessions, tasks, decisions, healingEvents, memories] = await Promise.all([
