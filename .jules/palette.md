...
## 2026-06-04 - Accessible Dynamically Rendered Result Blocks
**Learning:** When updating a React frontend with dynamically rendered inline result blocks after async operations (like form submissions or server actions), adding generic border colors and success text isn't enough for screen readers. They won't announce the result automatically, and decorative icons can clutter the announcement.
**Action:** Always attach `role="status"` (or `role="alert"` for errors) and `aria-live="polite"` to the container to ensure assistive technologies announce the result. Decorative icons within these blocks must use `aria-hidden="true"`.
