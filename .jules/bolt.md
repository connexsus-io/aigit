...
## 2025-06-13 - Replace higher-order array methods and regex in string builders
**Learning:** Using higher-order array methods (`forEach`, `filter`) and inline regular expressions (like `.replace(/-/g, '')`) inside string concatenation loops (e.g., during Markdown or Mermaid DAG generation) creates massive closure allocation overhead and repetitive garbage collection. This severely impacts string builder performance as string operations compound.
**Action:** Always replace `.forEach()` and `.filter()` with standard `for...of` loops in tight iteration paths to minimize closure overhead. Furthermore, pre-calculate and cache repetitive regex operations into mapping arrays (`Map<string, string>`) outside the loops to avoid repeated execution overhead during string builder interpolation.
