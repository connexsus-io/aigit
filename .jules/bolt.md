...
## 2024-05-22 - Optimize Mermaid Graph String Iterations
**Learning:** In `context-server/src/docs/mermaid.ts`, `generateMermaidGraph` repeatedly generated Mermaid node names using string replacements on UUIDs inside multiple nested map arrays (e.g., `id.replace(/-/g, '')`). This operation combined with standard array iteration functions `.forEach()` repeatedly invoked within the main control loops resulted in significant performance drops. During an ad-hoc benchmarking script processing big arrays (thousands of items), graph generation took ~368ms.
**Action:** Always pre-process and cache repeating array mapping logic (like UUID manipulation functions) outside of the main iterations into memory variables like `Maps`. Additionally, replacing higher-order array map functions like `.forEach` to explicit loops significantly reduces execution overhead (e.g. time dropped down to ~105ms on a 4.5K item load execution benchmark).
