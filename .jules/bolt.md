...
## 2024-05-18 - CLI Dump Ledger Overhead
**Learning:** In the CLI sync command (`dumpContextLedger`), performing sequential independent database queries (projects, agents, sessions, tasks, decisions, etc.) incurs an N+1 query latency penalty where the total time is the sum of all I/O roundtrips ($O(N)$).
**Action:** Always group independent sequential read operations into a single `Promise.all` block. This reduces the latency bottleneck from the sum of all queries to the maximum of the longest query ($O(\max(T))$).
