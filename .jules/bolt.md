...
## 2024-05-01 - Caching Failure States
**Learning:** When refactoring to introduce local function-scoped caches (e.g., using `Map` to cache AST extraction results), ensure the cache explicitly handles and stores failure states (like exceptions). Caching an empty array (`[]`) instead of a true failure indicator (e.g., `null`) can alter the downstream application's error-handling parity. For example, treating a parse error as "no symbols found" instead of "could not parse" can cause valid code references to be incorrectly flagged as stale or drifted.
**Action:** When creating caches for operations that can fail, always cache a distinct failure token (`null` or `undefined`) in the `catch` block to preserve the original logical branches that skipped processing on errors. Ensure the map type definition accommodates this (e.g., `Map<string, T | null>`).
