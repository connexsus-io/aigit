...
## 2026-07-14 - Optimize array mapping and spread operations in nested arrays
**Learning:** Chained array methods (like `.map()`, `.filter()`) and spread operators (`...arr`) allocate intermediate array representations, executing the closures multiple times per element. Using these operations together to combine or iterate large subsets like `memories` and `decisions` in `history.ts` and `ledger.ts` causes significant garbage collection overhead and execution latency compared to single-pass `for` loops.
**Action:** When mapping over, copying, or checking properties of multiple lists (e.g. `memories` and `decisions`), avoid combining them with spread operations or chaining map/filter functions. Always opt for a standard `for` loop, updating the final result array or numeric counter directly in a single pass.
