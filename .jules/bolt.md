...
## 2024-05-24 - Optimize Markdown Generation Closures
**Learning:** In heavily used code like `generateArchitectureDocs` and `generateMermaidGraph`, iterating over tens of thousands of objects using higher order functions like `.forEach` and `.filter` triggered huge GC overhead due to closure allocations. Also `t.id.replace(/-/g, '')` executed regex inside these large loops, leading to repetitive allocation overhead.
**Action:** Use standard `for (let i = 0; ...)` loops over large lists and arrays to prevent intermediate closures. Additionally, when processing IDs like UUIDs repetitively for graph nodes/edges, process and cache them upfront in mapping arrays to avoid repeated string and regex overheads during multi-pass processing.
