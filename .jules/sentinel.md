...
## 2024-06-27 - [CRITICAL] Fix SQL Injection in raw database query
**Vulnerability:** The `attachVectorEmbeddings` function in `context-server/src/cli/sync.ts` used a dynamically passed `tableName` directly within a `$executeRawUnsafe` query without explicitly validating it.
**Learning:** Relying purely on TypeScript union types (e.g. `'Memory' | 'Decision'`) for dynamically constructed SQL strings provides no runtime protection against SQL injection, as type checking is stripped during compilation.
**Prevention:** Always implement strict, explicit runtime allowlist checks (e.g. `if (tableName !== 'Memory' && tableName !== 'Decision')`) before inserting variable table or column names into raw SQL execution like `$executeRawUnsafe`.
