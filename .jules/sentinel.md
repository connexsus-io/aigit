...
## 2026-05-20 - [Fix Timing Side-Channel in Security Token Validation]
**Vulnerability:** The `isMatchingToken` validation in the UI server performed an early return based on `provided.length !== expected.length`. This leaks the exact length of the expected token via timing differences.
**Learning:** Checking length before constant-time comparisons (like `timingSafeEqual`) completely breaks the constant-time guarantee by creating an observable side-channel.
**Prevention:** When comparing arbitrary length security tokens or passwords using `timingSafeEqual`, always hash both inputs using a cryptographic hash function (like SHA-256) first. This ensures both buffers passed to `timingSafeEqual` are always perfectly equal in length, eliminating length-based timing leaks.
