...
## 2024-11-20 - [MEDIUM] Error Information Leakage in API Endpoints
**Vulnerability:** Found multiple instances in `context-server/src/cli/commands/ui.ts` where internal application errors were directly exposed to the client via `res.status(500).json({ error: String(error) });`.
**Learning:** Directly sending `String(error)` or `error.stack` to an API client risks leaking sensitive internal application logic, file paths, or underlying database structure details (e.g., Prisma query specifics). This is a common form of Information Disclosure.
**Prevention:** Always log detailed internal errors to the server console or a structured logger (e.g., `console.error`) for debugging, and return safe, generic error messages like `'Internal Server Error'` to the API consumer.
