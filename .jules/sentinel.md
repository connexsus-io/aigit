...
## 2024-05-23 - Fix Timing Attack in Token Validation
**Vulnerability:** Timing side-channel attack during token validation, returning early on string length mismatch, allowing an attacker to enumerate token length.
**Learning:** `timingSafeEqual` alone is not sufficient if the inputs can have different lengths, as comparing lengths or creating buffers of varying lengths is not constant time. Both strings must be normalized to a uniform length before using `timingSafeEqual`.
**Prevention:** Hash both inputs using a cryptographic hash like `sha256` before comparing them. This ensures both buffers have identical lengths, neutralizing length-enumeration timing vulnerabilities.
