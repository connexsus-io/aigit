...
## 2024-05-28 - [MEDIUM] Error Handling Leaking Internal Traces
...
**Required Follow-up:** Rotate or revoke the historically committed PostHog project key and Sentry DSN in their provider consoles; removing literals from the repository does not invalidate credentials already present in Git history.
## 2024-11-06 - [HIGH] Hardcoded Port in CORS Configuration
...
## 2025-02-20 - [HIGH] Timing Attack Vulnerability in token verification
...
## 2024-11-06 - [CRITICAL] Fix SQL Injection in sync.ts
**Vulnerability:** The `attachVectorEmbeddings` function in `context-server/src/cli/sync.ts` constructed raw SQL queries using `$executeRawUnsafe` with dynamic `tableName` interpolation without explicit runtime validation. Relying solely on TypeScript union types (`'Memory' | 'Decision'`) for the `tableName` parameter is insufficient for runtime security and could lead to SQL injection if the function was called with untrusted input bypassing TypeScript checks.
**Learning:** TypeScript type safety does not exist at runtime. When dynamically constructing SQL queries using `$executeRawUnsafe` with table names (which cannot be parameterized in SQL), you must strictly validate the table name against an allowlist before execution.
**Prevention:** Always implement explicit runtime validation against a strict allowlist (e.g., checking if the table name strictly equals `'Memory'` or `'Decision'`) before executing raw SQL queries that require dynamic table names.
